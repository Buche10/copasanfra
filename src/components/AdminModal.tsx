...
export const AdminModal: React.FC<AdminModalProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-rose-900 via-rose-800 to-rose-950 p-6 rounded-3xl text-white shadow-xl">
        ...
        <div className="flex items-center space-x-2 bg-black/20 p-1.5 rounded-2xl border border-white/10">
          ...
          <button
            ...
            className={`px-4 py-2 text-xs font-bold rounded-xl transition-all ${
              activeTab === 'resultados' ? 'bg-white text-rose-900 shadow' : 'text-rose-200 hover:bg-white/10'
            }`}
          >
            Resultados
          </button>
      ...
      {activeTab === 'settings' && (
        <div className="space-y-6">
          {/* Card 1: Categorías activas / suspendidas */}
          <div className="glass-card rounded-3xl p-6 border border-slate-200 shadow-md space-y-4">
            <div className="space-y-1 border-b pb-4">
              <h3 className="font-black text-slate-900 text-lg flex items-center gap-2">
                <Trophy className="w-5 h-5 text-[#00A859]" /> Categorías del Campeonato
              </h3>
              <p className="text-xs text-slate-500">
                Suspende una categoría para ocultarla del público, la inscripción y los informes,
                <strong> sin borrar sus datos</strong>. Reactívala cuando quieras. Esto <strong>no modifica el calendario</strong>.
              </p>
            </div>
